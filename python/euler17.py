...
def getletters(i):
	...
	if len(string) >= 2 and string[-2] == '1':
		if string[-1] == '0':
			...
			length += 6 # == len('eleven') == len('twelve')
		elif string[-1] == '3' or string[-1] == '4' or string[-1] == '9' or string[-1] == '8':
			length += 8 # == len('thirteen') == len('fourteen') == len('nineteen') ==len('eighteen')
		elif string[-1] == '5' or string[-1] == '6':
			length += 7 # == len('fifteen') == len('sixteen')
		elif string[-1] == '7' :
			length += 9 # == len('seventeen')

	# deal with the ones and tens place
	else:

		if string[-1] in ('1', '2', '6'):
			length += 3 # == len('one') == len('two') == len('six')
		elif string[-1] in ('4', '5', '9'):
			length += 4 # == len('four') == len('five') == len('nine')
		elif string[-1] in ('3', '7', '8'):
			length += 5 # == len('three') == len('seven') == len('eight')

		# tens place
		if len(string) > 1:
			if string[-2] in ('2', '3' , '8', '9'):
				length += 6 # == len('twenty') == len('thirty') == len('eighty') == len('ninety')
			elif string[-2] in ('4', '5', '6'):
				length += 5 # == len('forty') == len('fifty') == len('sixty')
			elif string[-2] in ('7',):
				length += 7 # == len('seventy')

	# deal with the hundreds place
	if len(string) > 2:
		if string[-2:] != '00':
			if string[-3] in('1', '2', '6'):
				length += 13 # == len('one hundred and')
			elif string[-3] in ('3', '7', '8'):
				length += 15 # == len('three hundred and')
			elif string[-3] in ('4', '5', '9'):
				length += 14 # == len('four hundred and')
		else:
			if string[-3] in ('1', '2', '6'):
				length += 10 # == len('one hundred')
			elif string[-3] in ('3', '7', '8'):
				length += 12 # == len('three hundred')
			elif string[-3] in ('4', '5', '9'):
				length += 11 # == len('four hundred')
